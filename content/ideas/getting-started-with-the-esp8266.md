...
It works with both ESP-01 and ESP-01S.
It uses a CH340C chip and added ESP8266 auto-download circuit,
for downloading programs, updating firmware, and debugging.
It support Arduino IDE, Ai-thinker ESP8266 Flasher, and Espressif FLASH_DOWNLOAD_TOOLS.
Some of its features are:

* The programmer interface is a USB Type A connector.
* It has RESET tact switch, so you don't need to unplug / replug to re-flash it.
* No need connection, no need to press button, WiFi module will go into download mode automatically
* It can control RESET and GPIO0 of the ESP8266, making flashing from most flashing tools fully automatic and painless.

>**NOTE:** When I first used the Stemedu USB to ESP8266 ESP-01 programmer board,
>it did not come up as a `/dev/ttyUSB0` device.
>Apears the support for CH340 USB to serial adapter based devices has a conflict.
>I followed the advice of ["/dev/ttyUSB0 not present in Ubuntu 22.04"][153] to resolve it.
