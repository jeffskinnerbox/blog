...
Screen sharing, also known as desktop sharing, is the practice of sharing the contents of the screen with another device (or multiple devices) so all participants view, and possible, control the same screen.  Effectively all the participants are sharing the same login session and accessing the device simultaneously.
* **What is Remote Access? -**
Remote access is the ability of users to access a device or a network from any location. With that access, users can login, creating an new user session, and see a screen, manage files, etc. on the remote device.
...
So TigerVNC cannot be used to screen share the currently active local desktop session.
...
* [How to connect to a remote computer using VNC in Linux][JJ]
...
* [How to use the screen share feature in Ubuntu][HH]
...
* [XXFCE: Remote Desktop via X11vnc Through an SSH Tunnel][CC]
...
[Vino][DD] was a VNC server for the GNOME desktop environment, capable of screen sharing,
but the GNOME developers now recommend using [gnome-remote-desktop][EE] instead.
On the GNOME desktop,
open **Settings**, select **Sharing**, and enable **Remote Desktop**
(see [here][JJ] for more details).
This should automatically enable the service and you should immediately
be able to connect via the [Remote Desktop Protocol (RDP)][FF].
This is a GNOME-only solution intended for the enterprise market.
It appears you can [use other types of desktops][HH],
but you must switch to GNOME for the screen sharing features to work.

GNOME also has a recommented solution for for remote access client software,
that being [Remmina][II].
...
[X11vnc][BB] is a VNC Server program, but operates differently than most VNC servers.
...
For maximum felexability, I will use [noVNC][AA] for the VNC Viewer.
...
The web article ["XFCE: Remote Desktop via X11vnc Through an SSH Tunnel"][CC]
...
# has permision to use ssh access
$ sudo ufw status
Status: active

To                         Action      From
--                         ------      ----
22/tcp                     ALLOW       192.168.1.200
...
ssh -v jeff@192.168.1.19 -L 5900:localhost:5900 "x11vnc -forever -shared -display :0 -noxdamage -ncache 10 -ncache_cr -nopw"
#OR
ssh -v jeff@192.168.1.19 -L 5900:localhost:5900 "x11vnc -passwd <password> -forever -shared -display :0 -noxdamage -ncache 10 -ncache_cr -nopw"
```

The `x11vnc` [commandline option/parameters][GG] used above are:
...
* **-forever** Keep listening for more connections rather than exiting as soon as the first
client(s) disconnect.
...
* **-nopw** Disable the big warning message when you use x11vnc without some sort of password.
* **-passwd <password>** used for authentication (USE AT YOUR RISK ... might see it via `ps`)
* **-passwdfile <filename>** Specify the password via the first line of the file <filename> (instead
of via -passwd on the command line where others might see it via `ps`).
* **-ncache <n>** Client-side caching scheme for framebuffer memory <n> (an integer) times that of the
full display screen contents for rapid retrieval.
<n> = 10 is the default and use 0 to disable.
* **-ncache_cr** In -ncache mode, try to do copyrect opaque window moves/drags
instead of wireframes (this can induce painting errors).

Sources:

* [XFCE: Remote Desktop via X11vnc Through an SSH Tunnel][CC]
...
this is intended to be a perminate installation so that my desktop Linux envirnment, `desktop`,
...
Also need to use `lightdm` for its display manager (can't use the GNOME display manager 3).
* Local Client - Any Linux machine having `vnc` client.

>**NOTE:** In my permenate installation done in this section,
>Remote Server = `desktop` and Local Client = `chromebook`.
>This is the direct oposite of the
...
#### Step 1: Install Software, Configure, and Start X11vnc
```bash
...
# on the remove server, findout what display manager are you using
$ cat /etc/X11/default-display-manager
/usr/sbin/gdm3

# if display manager is 'gdm3', change the display manager over to lightdm
# and reboot to get the display manager working
...
We need to create the approprate files so `systemd` can manage X11vnc.
...
[Service]
Type=simple
ExecStart=/usr/bin/x11vnc -forever -display :%i -auth guess -passwd password
...
* [X11vnc Mnaual Page][GG]
* [X11vnc](https://wiki.archlinux.org/title/x11vnc)
* [How to Setup x11vnc on Ubuntu 22.04](https://linuxopsys.com/topics/setup-x11vnc-on-ubuntu)
* [ArchLinux: X11vnc](https://wiki.archlinux.org/title/x11vnc)
* [How to install x11vnc vnc server as a service on Ubuntu 20.04, for remote access or screen sharing][KK]
...
#### Step 2: xxx
There is a conflict with the GNOME screen lock, such that, you will not be able to VNC after the screen locks.
You'll need to [turn-off the GNOME screen lock][KK] or [replace it with `xscreensaver`][LL].
...
[How To Install Or Change Screensaver in Ubuntu?][LL]
...
#### Step X: xxx
One last test before calling it done is to reboot the system once again.
It’s important to check that you can log in from the login screen
and you can get past the screen lock.
...
# now bring up the screen sharing of the remote server via a browser using the url provide above
google-chrome http://desktop:6080/vnc.html?host=desktop&port=6080
```
...
[CC]:https://softsolder.com/2022/01/03/xfce-remote-desktop-via-x11vnc-through-an-ssh-tunnel/
[DD]:https://wiki.gnome.org/Projects/Vino
[EE]:https://wiki.gnome.org/Projects/Mutter/RemoteDesktop
[FF]:https://en.wikipedia.org/wiki/Remote_Desktop_Protocol
[GG]:https://manpages.ubuntu.com/manpages/kinetic/man1/x11vnc.1.html
[HH]:https://www.addictivetips.com/ubuntu-linux-tips/use-screen-share-in-ubuntu/
[II]:https://ubuntu.com/tutorials/access-remote-desktop#1-overview
[JJ]:https://www.redhat.com/sysadmin/vnc-screen-sharing-linux
[KK]:https://www.crazy-logic.co.uk/projects/computing/how-to-install-x11vnc-vnc-server-as-a-service-on-ubuntu-20-04-for-remote-access-or-screen-sharing
[LL]:(https://wethegeek.com/how-to-install-or-change-screensaver-in-ubuntu/)
...
Because it works at the [framebuffer][42] level, it is applicable to all windowing systems and applications,
...
I choose to [install TigerVNC on my Ubuntu][51] system.
...
Now the TigerVNC Server is running on the Chromebook providing a GUI login experiance
...
create the file `/etc/systemd/system/vncserver@.service` with the folowing contents:
...
We are now going to start up the VNC service, spcifically for display `:1`
(that is why you see the referance to `@1` in `vncserver@1.service`):
...
#### Step X: Disable Suspend / Hibernation Modes - DONE
...
nor prevents it from invoking the screensaver.

Source:

* [How to Disable Suspend and Hibernation Modes In Linux](https://www.tecmint.com/disable-suspend-and-hibernation-in-linux/)

#### Step X: Power Management / Screen Locking
* [xubuntu 16.04: how do I make it lock screen automatically after timeout](https://askubuntu.com/questions/818919/xubuntu-16-04-how-do-i-make-it-lock-screen-automatically-after-timeout)
...
Appears the Raspberry Pi communicty has setttle on RealVNC as ther goto platform for VNC Server.
